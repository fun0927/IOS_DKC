...
class WorkNoAViewController: UIViewController {
    ...
    let tfcustomNm: UITextField = {
        ...
        textField.placeholder = "거래처를 입력하세요".localized
    ...
    let util = Util()
    var workNoAList:[WorkReportworkNoAListDetailResponse] = []
    ...
    let tfordNm: UITextField = {
        let textField = UITextField()
        textField.placeholder = "공사명을 입력하세요".localized
        textField.font = UIFont(name: "NotoSansKR-Regular", size: 16)
        textField.backgroundColor = .white
        textField.layer.borderWidth = 1
        textField.layer.borderColor = UIColor(red: 0.854, green: 0.854, blue: 0.854, alpha: 1).cgColor
...
//        alert.view.bounds = CGRect(x: 0, y: 0, width: 320 + 20, height: 500);
    ...
    @objc func btnReadClicked(){
        var workReportworkNoAListRequest = WorkReportworkNoAListRequest()
        if let yyyymmddFr = lblLeftDate.text {
            workReportworkNoAListRequest.ordDateFr = yyyymmddFr
        }
        if let yyyymmddTo = lblRightDate.text {
            workReportworkNoAListRequest.ordDateTo = yyyymmddTo
        }
        if let customNm = tfcustomNm.text {
            workReportworkNoAListRequest.customNm = customNm
        }
        if let ordNm = tfordNm.text {
            workReportworkNoAListRequest.ordNm = ordNm
        }
        hud.textLabel.text = "Loading"
        hud.show(in: self.view)
        print("workReportworkNoAListRequest : \(workReportworkNoAListRequest)")
        
        ApiService.request(router: WorkReportApi.workNoAList(param: workReportworkNoAListRequest), success: { (response: ApiResponse<WorkReportworkNoAListResponse>) in
            self.hud.dismiss()
...
extension WorkNoAViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        case 0:
            ...
            cell.lblcsNum.text = "원가번호".localized
            cell.lblcustomNm.text = "거래처".localized
            cell.lblordNm.text = "공사명".localized
            
        default:
            cell.lblcsNum.text = workNoAList[indexPath.row].csNum
            cell.lblcustomNm.text = workNoAList[indexPath.row].customNm
            cell.lblordNm.text = workNoAList[indexPath.row].ordNm
...
//        print("selected : \(indexPath.row)")
        ...
        dismiss(animated: true, completion: {
            if let previousViewController = self.previousViewController as? WorkReportRegisterViewController {
                ...
                print("item : \(item)")
                previousViewController.getwWrkNoAListFromPopup(workNoAListItem: item)
            } else {

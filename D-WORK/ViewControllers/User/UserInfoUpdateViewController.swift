...
class UserInfoUpdateViewController: UIViewController, WKUIDelegate {
...
//                }
                ...
                if let data = result.data {
                    ...
                    if (data.editTelNo != nil && !data.editTelNo!.isEmpty) ||
                        (data.editAddr != nil && !data.editAddr!.isEmpty) {
                        self.tftelNo.isEnabled = false
                        self.tfKeyword.isEnabled = false
                        self.tfDetailAddress.isEnabled = false
                        self.btnFind.setDisable()
    ...
    @objc func btnSaveClicked(){
        ...
        if stDetailAddress.isEmpty && stAddress.isEmpty {
            let alert  = UIAlertController(title: "알림".localized, message: "나머지 주소를 입력해 주세요".localized, preferredStyle: .alert)
            alert.addAction(UIAlertAction(title: "확인".localized, style: .default, handler: nil))
            self.present(alert, animated: true, completion: nil)
            return
        }
        ...
        hud.textLabel.text = "Loading"
        hud.show(in: self.view)
        ApiService.request(router: UserApi.infoUpdate(param: UserInfoUpdateRequest(telNo: stTelNo, zipCd: zipCd, roadAddress: roadAddress, bname: bname, buildingName: buildingName, detailAddress: stDetailAddress)), success: { (response: ApiResponse<SurveyInfoDetailResponse>) in
            self.hud.dismiss()
            if let result = response.value {
                if let code = result.code, code == 101 {
                    print("code is 101")
    ...
    @objc func btnFindClicked(){
        let gitURL = URL(string: Environment.BASE_URL + "/addressPop.html?type=IOS")!
        let url = gitURL
...
extension UserInfoUpdateViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
...
extension UserInfoUpdateViewController: WKNavigationDelegate {
    
}
extension UserInfoUpdateViewController: WKScriptMessageHandler {
    func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        if let data = message.body as? [String: Any] {
            zipCd = data["zonecode"] as! String
            roadAddress = data["roadAddress"] as! String
            bname = data["bname"] as! String
            buildingName = data["buildingName"] as! String
            
            var viewAddr = roadAddress + " " + bname
            if !buildingName.isEmpty {
                viewAddr += " (" + buildingName + ")"
            }
            self.tfKeyword.text = viewAddr
            self.tfDetailAddress.text = ""

...
//        cvCalendar.heightAnchor.constraint(equalToConstant: CGFloat(cvHeight)).isActive = true
        ...
        cvInfoList.register(CardInfoListCollectionViewCell.self, forCellWithReuseIdentifier: CardInfoListCollectionViewCell.ID)
...
//        alert.view.bounds = CGRect(x: 0, y: 0, width: 320 + 20, height: 500);
    ...
    func getMasterList(yyyymmddFr:String, yyyymmddTo:String){
        ...
        ApiService.request(router: CardApi.infoList(param: CardInfoListRequest(yyyymmddFr: yyyymmddFr, yyyymmddTo: yyyymmddTo)), success: { (response: ApiResponse<CardInfoListResponse>) in
            self.hud.dismiss()
...
//                print("result : \(result)")
    ...
    @objc func btnAddClicked(){
        if let getVc = util.getViewControllerFromText(label: "법인카드 및 접대비 사전신청 상세".localized) {
...
extension CardViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: CardInfoListCollectionViewCell.ID, for: indexPath) as! CardInfoListCollectionViewCell
        switch indexPath.section {
        case 0:
            cell.backgroundColor = UIColor(red: 0.958, green: 0.958, blue: 0.958, alpha: 1)
            cell.lblreqDate.text = "신청일".localized
            cell.lblexPur.text = "지출목적".localized
            cell.lblforeAmtNum.text = "금액".localized
        ...
        default:
            ...
            let dt = infoList[indexPath.row].reqDate
            ...
            cell.lblexPur.text = infoList[indexPath.row].exPur
            cell.lblforeAmtNum.text = infoList[indexPath.row].foreAmtNum
            cell.lblstat.text = infoList[indexPath.row].stat
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if let getVc = util.getViewControllerFromText(label: "법인카드 및 접대비 사전신청 상세".localized) {
            if let getVc2 = getVc as? CardAddViewController {
                getVc2.stat = item.stat ?? ""
                getVc2.statCd = item.statCd ?? ""
                getVc2.cardInfo = item

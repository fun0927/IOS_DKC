//
//  WorkPlanResultViewController.swift
//  D-WORK
//
...
class WorkPlanResultViewController: UIViewController, DropDownViewProtocol {
    ...
    func getDivList(){
        ...
        ApiService.request(router: WorkPlanResultApi.divList , success: { (response: ApiResponse<WorkReportReqInfoListResponse>) in
            self.hud.dismiss()
...
//        divNmDropDown.stTitle = "내용을 선택하세요".localized
    ...
    func getInfoRead(){
        ...
        ApiService.request(router: WorkPlanResultApi.infoRead(param: WorkPlanReportInfoReadRequest(yyyymm: yyyymm, divCdParam: selectedDivCd)) , success: { (response: ApiResponse<WorkingInfoResponse>) in
            if let result = response.value {
    ...
    @objc func btnFindClicked(){
        ...
        hud.textLabel.text = "Loading"
        hud.show(in: self.view)
        ApiService.request(router: WorkPlanResultApi.infoList(param: WorkPlanReportInfoListRequest(yyyymm: yyyymm, divCdParam: selectedDivCd, searchDiv: searchDiv, startCd: selectedStartCd, endCd: selectedEndCd)) , success: { (response: ApiResponse<WorkPlanResultInfoListResponse>) in
            if let result = response.value {
                self.hud.dismiss()
...
extension WorkPlanResultViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
